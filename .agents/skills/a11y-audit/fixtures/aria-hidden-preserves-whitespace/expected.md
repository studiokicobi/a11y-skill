...
## Snapshot
- Target: fixtures/aria-hidden-preserves-whitespace
- Framework: html
...
**Location**: `fixtures/aria-hidden-preserves-whitespace/button.html:5`
**Issue**: aria-hidden="true" on focusable <button>. Remove aria-hidden or add tabindex="-1".
**Fix**:
```diff
- <button aria-label="Save   now" aria-hidden="true" data-id="x   y">Save   now</button>
+ <button aria-label="Save   now" data-id="x   y">Save   now</button>
```
