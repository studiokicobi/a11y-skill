...
## Snapshot
- Target: fixtures/tailwind-contrast-unmapped
- Framework: react
...
### 1. [WCAG 1.4.3] — Low-contrast Tailwind class
**Location**: `fixtures/tailwind-contrast-unmapped/Badge.tsx:5`
**Issue**: Tailwind class `text-amber-300` likely fails WCAG AA contrast on light backgrounds.
**Decision needed**: This Tailwind color class likely fails WCAG AA 4.5:1 and has no safe mapping to a compliant shade. Pick a darker shade in the same family or a different accessible color class.
**Current code**:
```
