...
**Do not invent replacement colors outside this table.** If a color isn't here, route the issue to Group 2 and ask the user for a brand-aligned alternative.
...
Assumes background is white, gray-50, or gray-100. For darker backgrounds, route to Group 2.
...
- **Large text exception**: WCAG allows 3:1 for text ≥18pt (~24px) or ≥14pt bold (~18.66px bold). If the flagged text is large, a higher class like `-500` may pass. Route to Group 2 if unsure.
